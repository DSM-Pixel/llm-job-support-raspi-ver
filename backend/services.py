...
def dashboard_stats() -> dict:
    ...
    return {
        "backend": BACKEND,
        "stats": [
            {"icon": "▱", "delta": "↗ +124", "value": "2,748", "label": "색인 문서·소스", "sub": "청크 12,840"},
            {"icon": "◇", "delta": "↗ +3.9K", "value": "12,840", "label": "라벨 데이터셋", "sub": "검수 100%"},
            {"icon": "◎", "delta": "↗ +0.04", "value": "0.871", "label": "도로 파손 mAP@0.5", "sub": "YOLOe-L + SAM"},
            {"icon": "⌁", "delta": "↗ +18%", "value": "1,206", "label": "오늘 처리 작업", "sub": "질의·탐지·생성"},
        ],
        "weekly": [
            {"day": "월", "value": 42}, {"day": "화", "value": 64}, {"day": "수", "value": 48},
            {"day": "목", "value": 76}, {"day": "금", "value": 58}, {"day": "토", "value": 70},
            {"day": "일", "value": 52},
        ],
        "models": [
            {"name": "YOLOe-L 도로파손", "kind": "탐지", "load": 74, "state": "운영", "tone": "green"},
            {"name": "SAM 분할", "kind": "세그멘테이션", "load": 56, "state": "운영", "tone": "green"},
            {"name": "gemini-2.5-flash", "kind": "VLM·생성", "load": 40, "state": "운영", "tone": "green"},
            {"name": "도로파손 v3 파인튜닝", "kind": "학습", "load": 88, "state": "학습 중", "tone": "orange"},
        ],
        "activity": [
            {"icon": "⌗", "text": "pothole_set_2025Q2 라벨링 검수 완료", "meta": "김연우 · 12분 전"},
            {"icon": "⌕", "text": "공공데이터포털 ‘도로 파손 신고 현황’ 연계 동기화", "meta": "시스템 · 34분 전"},
            {"icon": "⇱", "text": "‘도로 파손 현황 분석 보고서’ 생성", "meta": "박서준 · 1시간 전"},
            {"icon": "▱", "text": "CCTV 이상행동 데이터셋 304건 업로드 완료", "meta": "이지은 · 3시간 전"},
        ],
    }
...
def route_query(question: str) -> dict:
    ...
    if re.search(r"포트홀|이미지|라벨|영역|박스", text):
        return {
            "backend": BACKEND, "intent": "image",
            "paragraphs": ["이미지 분석 작업으로 연결할 수 있습니다. 현재 요청은 <b>포트홀 위치 탐지와 라벨링</b>에 가장 적합합니다."],
            "steps": [
    ...
    if re.search(r"보고서|요약|리포트|문서", text):
        return {
            "backend": BACKEND, "intent": "report",
            "paragraphs": ["보고서 생성 요청으로 이해했습니다. 선택된 데이터 소스를 바탕으로 <b>요약, 통계 표, 출처</b>가 포함된 문서를 구성할 수 있습니다."],
            "steps": [
    ...
    return {
        "backend": BACKEND, "intent": "general",
        "paragraphs": [
...
_PRESET_FINDINGS = {
    "도로 파손/포트홀 찾기": [
        {"grade": "상", "tone": "red", "class_name": "포트홀",
         "note": "좌측 하단. 지름 약 35cm, 깊이 추정 6cm. 즉시 보수 대상."},
        {"grade": "중", "tone": "orange", "class_name": "포트홀",
         "note": "중앙. 지름 약 18cm. 차량 손상 우려, 7일 이내 보수."},
        {"grade": "중", "tone": "orange", "class_name": "선형 균열",
         "note": "우측 상단으로 진행. 표면 실링 권장."},
    ],
    "이미지 전체 설명": [
        {"grade": "정보", "tone": "gray", "class_name": "장면",
    ...
    "이상 상황 탐지": [
        {"grade": "주의", "tone": "orange", "class_name": "노면 침하",
         "note": "우측 하단 의심 영역. 추가 촬영 권장."},
    ],
}


def labeling_detect(preset: str = "도로 파손/포트홀 찾기", custom_prompt: str = "",
                    min_conf: int = 0) -> dict:
    ...
    if custom_prompt.strip():
        findings = [{
            "grade": "분석", "tone": "gray", "class_name": "사용자 질문 응답",
...
# 5. 보고서 생성 (MOCK)
# ────────────────────────────────────────────────────────────────────
def generate_report(report_type: str = "현황 분석", period: str = "최근 3년",
...
def list_datasets() -> dict:
    ...
    return {
        "backend": BACKEND,
        "datasets": [
            {"name": "pothole_set_2025Q2", "kind": "라벨", "count": "4,210", "fmt": "COCO JSON", "state": "검수 완료", "tone": "green", "date": "2026. 6. 16.", "owner": "김연우"},
            {"name": "road_raw_2026Q1", "kind": "원본", "count": "18,402", "fmt": "JPG", "state": "정제 중", "tone": "orange", "date": "2026. 6. 14.", "owner": "이지은"},
            {"name": "도로 파손 신고 현황", "kind": "공공데이터", "count": "38,402", "fmt": "OpenAPI", "state": "연계됨", "tone": "blue", "date": "2026. 5. 28.", "owner": "시스템"},
            {"name": "포트홀_보수_기준.md", "kind": "문서", "count": "5 청크", "fmt": "Markdown", "state": "색인됨", "tone": "green", "date": "2026. 5. 20.", "owner": "박서준"},
            {"name": "cctv_anomaly_2026", "kind": "원본", "count": "304", "fmt": "MP4 프레임", "state": "검수 대기", "tone": "gray", "date": "2026. 6. 10.", "owner": "이지은"},
        ],
    }
...
def upload_dataset(name: str = "") -> dict:
    """업로드 대기열 추가. (MOCK)"""
    return {"backend": BACKEND, "queued": True, "message": f"업로드 대기열에 추가되었습니다{f' — {name}' if name else ''}."}
